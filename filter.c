...
int filter_word(char *oriStr){
    ...
    if( mysql_real_query(&mysql,query,(unsigned int)strlen(query)) )//执行指定为计数字符串的SQL查询
    {
...
int filter_url(char *oriStr){
    ...
    if (res) {
        ...
        while ((row = mysql_fetch_row(res))) {
            printf("Fetched data...%s\n",row[1]) ;
            testURL[i]=calloc(strlen(row[1])+1, sizeof (char));
            strcpy(testURL[i], row[1]);
            i++;
            wpnum++;
        }
        if (mysql_errno(&mysql)) {
